// pages/chargingPlace/chargingPlace.js
...
Page({
  ...
  createMarker(point) {
    ...
    let marker = {
      iconPath: "../../public/image/tool1.png",
      id: point.id,
      latitude: latitude,
      longitude: longitude,
      label: {
  ...
  rList() {
    ...
    var market = [];
    util.http('battery/list', {}, 'get').then(res => {
      if (res.code == 200) {
        if (res.data != '') {
  ...
  bindmarkert(e) {
    ...
    util.http('battery/getOne', { id: id }, 'post', token).then(res => {
      if (res.code == 200) {
        this.setData({
